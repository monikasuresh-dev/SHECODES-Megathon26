...
def detect_emotion_and_intent(message: Optional[str]) -> Dict[str, Any]:
    """Classify conversational emotion and intent without diagnosing the patient."""
    text = (message or "").strip().lower()
    if not text:
        return {"emotion": "NEUTRAL", "confidence": 1.0, "intent": "conversation"}

    patterns = (
        ("EMERGENCY", ("help me", "can't breathe", "chest pain", "bleeding", "fell down", "call an ambulance"), "safety_support"),
        ("LONELY", ("alone", "nobody is here", "no one is here", "nobody cares", "wish someone would talk"), "emotional_support"),
        ("SAD", ("feel sad", "very sad", "not happy", "unhappy", "miss her", "miss him"), "emotional_support"),
        ("AFRAID", ("scared", "afraid", "frightened", "terrified"), "reassurance"),
        ("WORRIED", ("worried", "worry", "what will happen", "something will happen"), "reassurance"),
        ("FRUSTRATED", ("frustrated", "nothing is working", "annoyed", "fed up"), "emotional_support"),
        ("ANGRY", ("angry", "mad", "upset with"), "emotional_support"),
        ("CONFUSED", ("don't know where", "do not know where", "don't understand", "what is happening", "lost"), "grounding"),
        ("HAPPY", ("happy", "glad", "wonderful", "thank you", "love this"), "conversation"),
    )
    for emotion, phrases, intent in patterns:
        if any(phrase in text for phrase in phrases):
            return {"emotion": emotion, "confidence": 0.88, "intent": intent}

    if re.search(r"\b(what is my name|who am i|what do i|what food|who is|when do i|what song)\b", text):
        return {"emotion": "NEUTRAL", "confidence": 0.84, "intent": "memory_retrieval"}
    return {"emotion": "NEUTRAL", "confidence": 0.7, "intent": "conversation"}
...
class IntelligenceEngine:
    ...
    def process_message(
        ...
        emotion_eval = detect_emotion_and_intent(patient_message)
        return {
            ...
            "emotion": emotion_eval["emotion"],
            "emotion_confidence": emotion_eval["confidence"],
            "intent": emotion_eval["intent"],
        }
