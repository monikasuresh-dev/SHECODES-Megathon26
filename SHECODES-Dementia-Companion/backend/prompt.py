...
class PromptBuilder:
    ...
    def retrieve_relevant_memories(self, message: str) -> List[str]:
        """Return only memory facts related to the current utterance."""
        text = (message or "").lower()
        contacts = self.patient_profile.get("family_and_contacts", {})
        primary = contacts.get("primary_caregiver", {})
        secondary = contacts.get("secondary_contact", {})
        anchors = self.patient_profile.get("comfort_anchors", {})
        pet = anchors.get("cherished_pet", {})
        memories: List[str] = []
        if any(term in text for term in ("name", "who am i")):
            memories.append(f"Her name is {self.patient_profile.get('name', 'Lakshmi Narayanan')}.")
        if any(term in text for term in ("daughter", "ananya", "her")):
            memories.append(f"Ananya is her daughter and visits around {primary.get('visit_schedule', '5:30 PM')}.")
        if any(term in text for term in ("son", "ravi", "bengaluru")):
            memories.append(f"Ravi is her son and calls from {secondary.get('location', 'Bengaluru')}.")
        if any(term in text for term in ("dog", "pet", "muthu")):
            memories.append(f"Muthu is her beloved dog and is safe at Ananya's house.")
        if any(term in text for term in ("food", "eat", "hungry", "breakfast", "lunch", "sweet")):
            memories.append("She enjoys idli, dosa, sambar, rasam, lemon rice, curd rice and homemade sweets.")
        if any(term in text for term in ("music", "song", "carnatic", "tamil")):
            memories.append("She enjoys Carnatic music and old Tamil film songs.")
        if any(term in text for term in ("wake", "morning", "routine", "time")):
            memories.append("Her morning begins at 6:30 AM with warm water, prayer and devotional music.")
        if any(term in text for term in ("home", "where am i", "place", "beach", "temple")):
            memories.append("Her familiar home is in Anna Nagar, Chennai.")
        return memories[:4]
    ...
    def get_system_prompt(self) -> str:
        ...
        return (
            ...
            f"2. Match Length to Feeling: 1-3 short sentences when calm; 2-4 for confusion; 3-6 for sadness, loneliness, worry, or fear. Keep every sentence simple.\n"
            ...
            f"9. Context: Use the recent conversation and only the relevant memories supplied for this turn. Never invent facts.\n"
            f"10. Quality check: Address the complete message, name the feeling when clear, validate it, and offer one gentle next step."
        )
    ...
    def build_turn_prompt(
        ...
        emotion = intelligence_result.get("emotion", "NEUTRAL") if intelligence_result else "NEUTRAL"
        intent = intelligence_result.get("intent", "conversation") if intelligence_result else "conversation"
        memories = self.retrieve_relevant_memories(sanitized_message)
        ...
        prompt = (
            ...
            f"{directive_text}"
            f"CURRENT EMOTION: {emotion}. INTENT: {intent}.\n"
            f"RELEVANT PATIENT MEMORIES (use only if helpful): {memories or 'None'}\n\n"
            f"{history_context}"
